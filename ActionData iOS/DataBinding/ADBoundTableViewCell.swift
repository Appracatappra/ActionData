...
/**
 Displays a cell of bound data for a `ADBoundTableView`. Typically, a `ADBoundTableViewCell` is generated by a `ADBoundTableViewDataSource`. A `ADBoundTableViewCell` provides a method to attach it to a data model (any Swift class or structure that conforms to the `Codable` protocol) and any control conforming to the `ADBindable` protocol on any **View** or **SubView** will automatically be populated with the values from the data model based on the `dataPath` property of the control. The `dataPath` must match the name and case of a field on the attached data model or be a formula using a SQL like syntax.
 
 By using the `setDataModel` function to set the model and automatically populate the fields, you to not need to create **Actions** or **Outlets** for the individual controls. The `ADBoundTableViewCell` will automatically handle the reading and writing of properties for you.
 ...
 The `ADBoundTextPicker` control has an embedded picker view that will be handled automatically by the `ADBoundTableViewCell`. An accessory toolbar will be added to the picker that can be customized with the `cancelButtonText`, `cancelButtonImage`, `doneButtonText` and `doneButtonImage` properties of the `ADBoundTableViewCell`.
 ...
 */
open class ADBoundTableViewCell: UITableViewCell, ADBindingController, ADBindingDetailController {
